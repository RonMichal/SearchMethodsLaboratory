...
from typing import Iterable, Union, TypeVar, Generic, Tuple
import time
...
class AStar(ABC, Generic[T]):
    ...
    ) -> Tuple[Union[Iterable[T], None], int, float, float]:
        starting_time = time.time()
        expanded = 1
        if self.is_goal_reached(start, goal):
            return [start], expanded, time.time() - starting_time, 0
        ...
        while openSet:
            ...
            if self.is_goal_reached(current.data, goal):
                return self.reconstruct_path(current,
                                             reversePath), expanded, time.time() - starting_time, current.gscore
            ...
            for neighbor in map(lambda n: searchNodes[n], self.neighbors(current.data)):
                ...
                if neighbor.out_openset:
                    ...
                    heappush(openSet, neighbor)
        return None, expanded, time.time() - starting_time, Infinite
